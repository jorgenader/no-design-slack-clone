...
def defaults():
    ...
    env.code_dir = '/srv/no_design_slack_clone'
        ...
        "letsencrypt": [
            {
                ...
                "filename": "letsencrypt.no_design_slack_clone.conf",
        ...
        "nginx": [
            {
                ...
                "filename": "app.no_design_slack_clone.include",
                "remote_path": "/var/lib/docker-nginx/config/conf.d/%(filename)s"
            },
            {
                "pattern": "%(target)s.ssl",
                "filename": "ssl.no_design_slack_clone.include",
                "remote_path": "/var/lib/docker-nginx/config/conf.d/%(filename)s"
            },
            {
                "default_site": True,
                "pattern": "%(target)s.conf",
                "filename": "no_design_slack_clone",
...
def deploy(id=None, silent=False, force=False, auto_nginx=True):
    ...
    app_patterns = [r' no_design_slack_clone/app', r' no_design_slack_clone/static',
                    r' no_design_slack_clone/settings', r' no_design_slack_clone/package.json']
    ...
    if crontab_changed:
        with cd(env.code_dir):
            sudo('cp deploy/crontab.conf /etc/cron.d/no_design_slack_clone')
...
def setup_server(id=None):
    ...
    # Create database
    sudo('echo "CREATE DATABASE no_design_slack_clone; '
         '      CREATE USER no_design_slack_clone WITH password \'{db_password}\'; '
         '      GRANT ALL PRIVILEGES ON DATABASE no_design_slack_clone to no_design_slack_clone;" '
         '| docker exec -i postgres-9.5 psql -U postgres'.format(db_password=db_password))

    # Upload local settings
    put(local_path=StringIO(local_settings), remote_path=env.code_dir + '/no_design_slack_clone/settings/local.py', use_sudo=True)

    # Create log dir
    sudo('mkdir -p /var/log/no_design_slack_clone/')
    ...
    sudo('touch %s/no_design_slack_clone/app/webpack-stats.json' % env.code_dir)
    ...
    # Copy logrotate conf
    with cd(env.code_dir):
        sudo('cp deploy/logrotate.conf /etc/logrotate.d/no_design_slack_clone')
...
def docker_up(silent=False):
    ...
    # See https://trello.com/c/ymHxLkPq/236-research-nginx-proxy for more info.
    sudo('docker exec nginx nginx -s reload')
...
def collectstatic(npm_build=True):
    if npm_build:
        docker_compose_run('node', 'npm run build', name='no_design_slack_clone_npm_build')
...
def docker_compose_run(service, cmd='', name='no_design_slack_clone_tmp'):
